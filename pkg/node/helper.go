...
func CheckHostsAreRPCCompatible(app *application.Avalanche, hosts []*models.Host, subnetName string) error {
	...
	if len(incompatibleNodes) > 0 {
		...
		case models.CustomVM:
			ux.Logger.PrintToUser("To modify your Custom VM binary: metal blockchain upgrade vm %s --config", subnetName)
		}
		ux.Logger.PrintToUser("Yoy can use \"metal node upgrade\" to upgrade Avalanche Go and/or Subnet-EVM to their latest versions")
